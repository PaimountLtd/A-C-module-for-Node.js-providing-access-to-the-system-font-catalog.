...
FontDescriptor *createFontDescriptor(FcPattern *pattern)
{
	...
	return new FontDescriptor((char *)path, (char *)psName, (char *)family, (char *)style, convertWeight(weight), convertWidth(width), slant == FC_SLANT_ITALIC,
				  slant == FC_SLANT_OBLIQUE, spacing == FC_MONO);
}
