...
FontDescriptor *resultFromFont(IDWriteFont *font)
{
	...
	if (numFiles > 0) {
		...
		if (SUCCEEDED(hr)) {
			...
			res = new FontDescriptor(psName, postscriptName, family, style, (FontWeight)font->GetWeight(), (FontWidth)font->GetStretch(),
						 font->GetStyle() == DWRITE_FONT_STYLE_ITALIC, font->GetStyle() == DWRITE_FONT_STYLE_OBLIQUE, monospace);
...
FontDescriptor *findFont(FontDescriptor *desc)
{
	...
	if (fonts->size() == 0) {
		...
		FontDescriptor *fallback = new FontDescriptor(NULL, NULL, NULL, NULL, desc->weight, desc->width, desc->italic, desc->oblique, false);
...
public:
	...
	(void *clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY, DWRITE_MEASURING_MODE measuringMode, DWRITE_GLYPH_RUN const *glyphRun,
	...
	(void *clientDrawingContext, FLOAT originX, FLOAT originY, IDWriteInlineObject *inlineObject, BOOL isSideways, BOOL isRightToLeft, IUnknown *clientDrawingEffect)
	{
		return E_NOTIMPL;
	}
...
FontDescriptor *substituteFont(char *postscriptName, char *string)
{
	...
	if (font) {
		...
		HR(factory->CreateTextFormat(familyName, collection, (DWRITE_FONT_WEIGHT)font->weight, font->italic ? DWRITE_FONT_STYLE_ITALIC : DWRITE_FONT_STYLE_NORMAL,
		...
		HR(factory->CreateTextFormat(L"", collection, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 12.0, L"en-us", &format));
	}
